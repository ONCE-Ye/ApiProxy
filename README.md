...
一个基于 Next.js 14 的目录站点，用来分开展示智能体官方入口与第三方 API 供应商信息，便于快速核对可登录渠道、官方 API 和第三方接入平台。

## 当前范围

- `智能体`：只展示官方登录入口、官方 API、基础说明
- `第三方供应商`：单独展示供应商简介、支持的智能体、文档与控制台入口
- 搜索与筛选：按名称、标签、区域快速过滤
- 详情页：分别提供智能体详情页和供应商详情页
...
- `data/providers.ts`：第三方供应商信息
- `components/agent-directory.tsx`：首页主目录界面

## 数据维护规则

1. 不要把第三方供应商重新混入智能体官方卡片。
2. 智能体数据里优先维护 `officialLogin` 和 `officialApi`。
3. 供应商数据里维护 `supportedAgentSlugs`、`docsUrl`、`consoleUrl` 和说明文本。
